...
export function Slider() {
  ...
  return (
    <div className="w-1/2 ">
      <div className="h-2/6">
        <nav className="grid grid-cols-3 grid-rows-2 gap-4">
          {stacks.map(stack => (
            <button
              key={stack.id}
      ...
      <div className="h-4/6 relative">
        {stacks.map(stack => (
          <div
            key={stack.id}
            className={`absolute bottom-0 right-0 w-[400px] h-[350px] 2xl:w-[500px] 2xl:h-[500px] transition-opacity duration-300 px-4 ${activeStack === stack.id ? 'opacity-100' : 'opacity-0'}`}
          >
            <img
